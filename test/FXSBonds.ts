...
const btrflyValueUSD = BigNumber.from(3320)

const FXS_WHALE = '0xee8ad6769fe89ecb8fee0d981ad709e08e6d1c06'
...
describe('Live FXS bonds', function () {
  ...
  await treasuryContract.toggle(
    ...
    await treasuryContract.connect(treasuryOwner).queue(BigNumber.from(2), fxs.address)
    await treasuryContract
      .connect(treasuryOwner)
      .toggle(BigNumber.from(2), fxs.address, ZERO_ADDRESS)
    await treasuryContract.connect(treasuryOwner).setFloor(fxs.address, '26143790')
  })

  it(`InitialDebt of ${INITIALDEBT} gives a ROI between 5% & 10% out the gate`, async function () {
